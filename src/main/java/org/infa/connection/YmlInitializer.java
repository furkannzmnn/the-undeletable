...
public final class YmlInitializer implements FileInitializer<Connection> {
    ...
    @Override
    public FileResponse<Connection> initialize(String path) throws IOException {
        ...
        final Connection connection = mapper.readValue(new File(path), Connection.class);

        LOGGER.info("connection: " + connection.getKafkaConnection().getTopic());

        return new FileResponse<>(connection);
    }
}
